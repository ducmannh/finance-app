...
export function TransactionDialog({
  ...
  // Lọc danh mục phù hợp theo loại Thu nhập / Chi tiêu
  const filteredCategories = categories.filter((c) => c.type === selectedType);

  useEffect(() => {
    if (transaction) {
      const initialD = new Date(transaction.date);
      setValue("type", transaction.type);
      setValue("amount", transaction.amount);
      setValue("categoryId", transaction.categoryId);
      setValue("date", initialD);
  ...
  // Tự chọn danh mục đầu tiên nếu đổi loại Thu/Chi
  useEffect(() => {
  ...
  }, [selectedType, filteredCategories, selectedCategory, setValue, transaction]);
  ...
  const onSubmit = async (data: CreateTransactionInput) => {
    ...
    try {
      let res;
      if (transaction) {
        res = await updateTransactionAction({ id: transaction.id, ...data });
      } else {
  ...
  return (
    <>
      <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 animate-in fade-in-0">
        <div className="bg-card text-card-foreground border border-border/60 shadow-2xl rounded-2xl max-w-lg w-full overflow-hidden p-6 space-y-6 animate-in zoom-in-95 max-h-[90vh] overflow-y-auto">
          {/* Header */}
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
            ...
            <div className="space-y-2">
              ...
              {filteredCategories.length === 0 ? (
                ...
                <div className="grid grid-cols-3 sm:grid-cols-4 gap-2 max-h-40 overflow-y-auto p-1 border border-border/50 rounded-xl">
                    ...
                    return (
                      <button
                        ...
                        className={`p-2.5 rounded-xl border flex flex-col items-center gap-1.5 transition-all text-xs font-medium ${
                          isSelected
                            ? "border-primary bg-primary/10 text-primary font-bold ring-2 ring-primary/20 scale-105"
            ...
            <div className="space-y-2">
              <Label htmlFor="transactionDate" className="text-sm font-medium flex items-center gap-1.5">
